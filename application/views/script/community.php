<script>
	$(document).ready(function() {
		...
		function community_admin() {
			$.ajax({
				url : "<?php echo base_url('api/Komunitas/admin') ?>",
				method : "POST",
				data : {
					komunitas_id : <?php echo $this->session->userdata('komunitas_id'); ?>
				},
				success : function(req) {
					if (req.error==true) {
						...
						$('tbody#community-admin').html(html);
					}
				}
			})
		}
									...
									<div class="row">\
										<div class="col-md-3">\
											<div style="padding: 5%; margin-top: 2px">\
												<img src="<?php echo base_url('assets/app/media/img/products/product'); ?>'+obj.image+'" style="width: 100%" class="rounded border">\
											</div>\
										</div>\
										<div class="col-md-9">\
			...
			$.ajax({
				...
				success : function(req) {	
					$('div.foto-identitas').html('\
						<img src="<?php echo base_url('assets/img/komunitas/'); ?>'+req.data[0].foto_identitas+'" style="width: 100%; padding: 5px" class="rounded shadow-sm">\
						');
				}
			})
		}
		...
		$(document).on('click','button.btn-join',function() {
			$.ajax({
				...
				method : "POST",
				data : {
					user_id : '<?php $this->session->userdata('id'); ?>',
